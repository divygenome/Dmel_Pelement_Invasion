...
{
  h$time <- factor(h$time, levels=c("G0", "G6", "G15", "G21", "G30", "G40", "G50", "G100"))
...
We then used this annotation file with featurecounts for assessing expression levels in all mapped bam files, using the following command (make sure featurecounts is active in the current environment).
...
colnames(data)[grepl(".sort.bam", colnames(data))] <- gsub("dmel_(R[0-9]+G[0-9]+)_run*", "\\1", colnames(data)[grepl(".sort.bam", colnames(data))])
...
long_data <- gather(subset_data, key = "Replicate_Generation", value = "Counts", R1G0:R3G100)
