...
truncation_pairs <- combined_data %>%
  ...
  summarise(
    trunc_pairs = list({
      ...
      tibble(
        trunc_s = (left_truncs[1:num_pairs]),
        trunc_e = (right_truncs[1:num_pairs])
      )
    }),
    .groups = "drop"
  ) %>%
...
truncation_pairs <- truncation_pairs %>%
  mutate(
    trunc_start = trunc_start + 1,
    trunc_end = trunc_end - 1
    )
...
average_hq_cov <- combined_data %>%
  ...
  summarise(avg_hq_cov = mean(hq_cov, na.rm = TRUE), .groups = "drop") # Drop grouping

# Update visualization
plot_avg <- ggplot(average_hq_cov, aes(x = pos, y = avg_hq_cov)) +
  geom_line(size = 1, color = "black") + # Add '+' to continue the ggplot chain
  # Shade merged truncation windows
  geom_rect(
    data = truncation_pairs,
    aes(
      xmin = trunc_start, 
      xmax = trunc_end, 
      ymin = -Inf, 
  ...
  facet_wrap(~ replicate, scales = "fixed") + # Keep facets for replicates
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(), # Remove major gridlines
    panel.grid.minor = element_blank(), # Remove minor gridlines
...
# Save plot
ggsave("figs/dmel_deviate_avg.png", plot_avg, width = 26, height = 5, dpi = 400)
ggsave("figs/dmel_deviate_avg.svg", plot_avg, width = 26, height = 5)
...
combined_data_with_truncs <- combined_data %>%
  left_join(truncation_pairs, by = "replicate") %>%
  mutate(
...
# Order the combined_avg_hq_cov table by replicate and coverage_ratio
combined_avg_hq_cov <- combined_avg_hq_cov %>%
  arrange(replicate, coverage_ratio)
...
# Print the ordered table for verification
print(head(combined_avg_hq_cov))
